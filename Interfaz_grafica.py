...
class LoginScreen(ttk.Frame):
    ...
    def login(self):
        ...
        if not user or not pwd:
            ...
            db_user = backend.validar_login(user, pwd)
            if db_user:
...
class UserManagement(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        self.e_id = ttk.Entry(frm_left, state="disabled") 
    ...
    def on_show(self):
        # Limpiar y recargar desde SQLite
        for item in self.tree.get_children():
            self.tree.delete(item)
        usuarios = backend.obtener_usuarios()
        for u in usuarios:
            self.tree.insert("", "end", values=(u['id_usuario'], u['nombre_completo'], u['num_identificacion'], u['grupo']))

    def registrar(self):
        messagebox.showinfo("Registro Exitoso", "Función en construcción - El usuario será registrado en BD.")
        
    def eliminar(self):
        respuesta = messagebox.askyesno("Confirmación", "¿Está seguro de eliminar este usuario?")
        if respuesta:
            messagebox.showinfo("Eliminación Confirmada", "En construcción.")
...
class MaterialManagement(ttk.Frame):
    ...
    def on_show(self):
        for item in self.tree.get_children():
            self.tree.delete(item)
        materiales = backend.obtener_materiales()
        for m in materiales:
            self.tree.insert("", "end", values=(m['codigo_material'], m['nombre_material'], m['cantidad_disponible'], m['cantidad_total']))

    def registrar(self):
        messagebox.showinfo("Registro Exitoso", "Material registrado en construcción.")
...
class LoanScreen(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        self.cmb_user = ttk.Combobox(frm_form, state="readonly", width=40)
        ...
        self.cmb_material = ttk.Combobox(frm_form, state="readonly", width=40)
        ...
        self.e_date_loan = ttk.Entry(frm_form, state="normal", width=43)
        ...
        if DateEntry:
            self.date_return = DateEntry(frm_form, width=40, background='darkblue', foreground='white', borderwidth=2, date_pattern='yyyy-mm-dd')
            self.date_return.grid(row=3, column=1, pady=10, padx=10)
        else:
            self.date_return = ttk.Entry(frm_form, width=43)
    ...
    def on_show(self):
        ...
        usuarios = backend.obtener_usuarios()
        self.usuarios_dict = {f"{u['num_identificacion']} - {u['nombre_completo']}": u['id_usuario'] for u in usuarios}
        self.cmb_user['values'] = list(self.usuarios_dict.keys())
        
        materiales = backend.obtener_materiales()
        self.materiales_dict = {f"{m['codigo_material']} - {m['nombre_material']} (Disp: {m['cantidad_disponible']})": m['id_material'] for m in materiales}
        self.cmb_material['values'] = list(self.materiales_dict.keys())
        
        self.cmb_user.set("")
        self.cmb_material.set("")

    def registrar(self):
        user_sel = self.cmb_user.get()
        mat_sel = self.cmb_material.get()
        
        if not user_sel or not mat_sel:
            messagebox.showerror("Error", "Seleccione usuario y material.")
            return
            
        id_user = self.usuarios_dict[user_sel]
        id_mat = self.materiales_dict[mat_sel]
        
        fecha_vto = self.date_return.get() if not isinstance(self.date_return, ttk.Entry) else self.date_return.get()
        
        exito, msj = backend.registrar_prestamo(id_user, id_mat, fecha_vencimiento=fecha_vto)
        
        if exito:
            messagebox.showinfo("Éxito", msj)
            self.on_show() # Recargar
        else:
...
class ReturnScreen(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        ttk.Label(frm_form, text="Préstamo Activo:").grid(row=0, column=0, sticky="e", pady=10, padx=10)
        self.cmb_loan = ttk.Combobox(frm_form, state="readonly", width=60)
        ...
        self.e_date_return = ttk.Entry(frm_form, state="normal", width=63)
        self.e_date_return.insert(0, datetime.now().strftime("%Y-%m-%d"))
        self.e_date_return.configure(state="disabled")
    ...
    def on_show(self):
        prestamos = backend.obtener_prestamos_activos()
        self.prestamos_dict = {f"PR-{p['id_prestamo']} | {p['nombre_completo']} -> {p['nombre_material']} (Vence: {p['fecha_vencimiento']})": p['id_prestamo'] for p in prestamos}
        self.cmb_loan['values'] = list(self.prestamos_dict.keys())
        self.cmb_loan.set("")

    def registrar(self):
        sel = self.cmb_loan.get()
        if not sel:
            messagebox.showerror("Error", "Seleccione un préstamo para procesar la devolución.")
            return
            
        id_prestamo = self.prestamos_dict[sel]
...
class ReportScreen(ttk.Frame):
    ...
    def generar(self):
        messagebox.showinfo("Reporte Generado", f"El archivo PDF ha sido generado.\n(Generación en DB SQLite en construcción)")
        
    def exportar(self):
        messagebox.showinfo("Exportación Exitosa", f"Datos exportados a CSV/Excel correctamente.\n(Exportación desde DB SQLite en construcción)")
